...
[Collection("sql")]
public class StaffFeatureTests(PostgresFixture fx)
{
    ...
    [Fact]
    public async Task List_staff_returns_403_when_plan_lacks_staff_support()
    {
        ...
        var err = await Error(await client.GetAsync("/v1/staff"), HttpStatusCode.Forbidden);
        err.GetProperty("code").GetString().Should().Be("feature_locked");
    }

    [Fact]
    public async Task List_staff_ok_on_platinum_plan()
    {
        await using var app = App();
        var tenantId = Guid.NewGuid();
        ...
        (await client.GetAsync("/v1/staff")).StatusCode.Should().Be(HttpStatusCode.OK);
    }
}
